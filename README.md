...
These fourteen skills mirror a documentation-driven V-model workflow, named to a uniform
`<step>-spec` convention. Steps 1-10 are sequential phases of the V-model; `explore-spec`,
`research-spec`, `commit-spec`, and `audit-spec` are cross-cutting steps callable at any point in the flow.
...
| 5   | [`plan-spec`](./plan-spec/SKILL.md)                         | `specs/<feature-slug>/plan.md` (+ optional `data-model.md`, `research.md`) or `docs/plans/<topic-slug>.md` |
...
| -   | [`explore-spec`](./explore-spec/SKILL.md)                   | `.speckit/memory/exploration.md` (cross-cutting)                                                           |
...
2. **For cold-start repositories** (no existing docs), run `explore-spec` first to get a lightweight orientation report in `.speckit/memory/exploration.md`.
3. Start a feature by running `documentation-spec` (global view) and `specify-spec` (feature spec).
4. Use `clarify-spec` -> `plan-spec` -> `tasks-spec` -> `analyze-spec` before you implement. When planning a greenfield or cross-feature system, `plan-spec` can first emit a project-level guide in `docs/plans/<topic-slug>.md`.
5. Invoke `research-spec` whenever a stack choice, architecture direction, or open-source comparison needs stronger evidence; it can run before planning or later when a decision is disputed.
...
2. **Cold-start exploration.** `explore-spec` provides a lightweight orientation when no docs
   exist, producing module outlines rather than full API documentation.
3. **Templates are references, not straitjackets.** Agents respect each template's intent and
...
4. **Ask the user when in doubt.** Every skill carries the same escalation rule: if a fact
   cannot be grounded in the inputs, stop and ask the user in one consolidated message. No
   guessing, no silent assumptions.
5. **Spec includes both target behavior AND current implementation.** `specify-spec` records
   the requested feature *and* the existing code paths it will touch.
6. **Analyze validates against the documentation set.** `analyze-spec` checks plan/tasks for
   consistency with `spec.md` and project `docs/*.md`. The constitution is one input among
   many, not the only gate.
7. **Test-as-verification, not test-first dogma.** Unit and integration test skills exist as
   explicit steps but the agent is not forced into red/green order.
8. **Research is a first-class cross-cutting step.** `research-spec` may be invoked at any
...
9. **Checkpoint freely.** `commit-spec` is callable at any phase, not only at the end. All
...
10. **Audit is a first-class cross-cutting step.** Use `audit-spec` after implementation (and
